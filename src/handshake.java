...
public class handshake {
    ...
    public byte[] createHandshake() {
        ...
        try {
            ...
            ByteBuffer peerIdBuffer = ByteBuffer.allocate(PEER_ID_LENGTH).putInt(this.peerID);
            Logger.info("Handshake peer ID bytes: " + Arrays.toString(peerIdBuffer.array()));
            stream.write(peerIdBuffer.array());
            // Log handshake start event
            ConnectionEventLogger.peerHandshakeStarted(this.peerID, 29382374); // -1 indicating unknown peer ID
        } catch (IOException e) {
    ...
    public static int readHandshake(byte[] handshakeMessage) throws P2PFileSharingException {
        if (handshakeMessage.length != HEADER.length() + ZERO_BITS_LENGTH + PEER_ID_LENGTH) {
            throw new P2PFileSharingException("Handshake message is not the correct size.",
                    P2PFileSharingException.ErrorType.MESSAGE_ERROR);
        }

        String receivedHeader = new String(handshakeMessage, 0, HEADER.length(), StandardCharsets.UTF_8);
        if (!HEADER.equals(receivedHeader)) {
            throw new P2PFileSharingException("Handshake failed: Incorrect header.",
